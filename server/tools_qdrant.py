...
async def _qdrant_search_raw(
    ...
    """Internal Qdrant search returning a dict (no JSON serialization).

    Accepts an optional pre-computed embedding vector to avoid redundant API calls.
    """
...
async def qdrant_search(
    ...
    try:
        result = await _qdrant_search_raw(
            query=query,
            collection=collection,
            limit=limit,
            score_threshold=score_threshold,
